<template>
    <div id="bulk-submission" style="min-width: 1100px;">
        <el-form :label-position="labelPosition" label-width="80px" :model="formLabelAlign">
            <el-form-item label="课程ID" :rules="[
                { required: true, message: '不能为空'},
                { type: 'number', message: '必须为数字值'}
                ]">
                <el-input v-model="courseid"></el-input>
            </el-form-item>
            <el-form-item label="题目ID" :rules="[
                { required: true, message: '不能为空'},
                { type: 'number', message: '必须为数字值'}
                ]">
            ...
            <el-form-item label="Upload Time" :rules="[
                { required: true, message: '不能为空'},
                { type: 'number', message: '必须为数字值'}
                ]">
...
export default {
    ...
    methods: {
        ...
        handleCodeExceed(files, fileList) {
            Message.warning(`当前限制选择 20 个文件，本次共选择了 ${files.length + fileList.length} 个文件`);
        },
