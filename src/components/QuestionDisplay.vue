        ...
        <el-container style="height: 630px; border: 1px solid #eee">
          <el-aside width="50%">
            <el-container>
              <el-header>
                <h1
                  style="font-size: 25px; text-align: center; line-height: 1.7"
                >
                  {{ title }}
                </h1>
              </el-header>
              <el-main>
                <h4
                  style="font-size: 15px; text-align: center; line-height:1.0; color=#909399;"
                >
                  Time: {{ issueTime }} &nbsp;&nbsp; Creator: {{ creator }}
                </h4>
                <h4
                  style="font-size: 15px; text-align: center; line-height:1.0; color=#909399;"
                >
                  TimeLimit: {{ timeLimit }} ms &nbsp;&nbsp; MemoryLimit:
                  {{ memoryLimit }} MB
                </h4>
                <h4
                  style="font-size: 15px; text-align: center; line-height:1.0; color=#909399;"
          ...
          <el-main>
            ...
            <el-row>
              <h4
                style="font-size: 15px; text-align: left; line-height:1.0; color=#909399;"
              >
                {{ this.compileHint }}
              </h4>
            </el-row>
...
export default {
  ...
  computed: {
    ...
    compileHint() {
      return compileOptions[this.language]
        ? "编译选项: " + compileOptions[this.language]
        : "";
    },
  },
  methods: {
    ...
    initQuestionID() {
      if (
        ...
        this.isNumber(this.$route.query.id) === false
      ) {
        this.$router.replace({ path: "/404" });
        return 0;
      } else {
      ...
      axios({
        ...
        .then((res) => {
          ...
          if (res.status === 200 && res.data.code === 200) {
            ...
            this.$router.push({
              path: "/ojlab",
              query: { tabindex: 2, courseid: this.courseid },
            });
          } else {
